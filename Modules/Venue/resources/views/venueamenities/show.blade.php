            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-body">
                        ...
                        <div class="text-end">   
                        <a href = "{{ route('venueamenities/create') }}" class="btn btn-primary waves-effect waves-light mb-4 text-end">
                         ...
                         <div class="table-responsive">
                            ...
                            <table class="table mb-0">
                                ...
                                <tbody>
                                      @if(count($venueamenities) > 0)
                                      @php $i=1; @endphp
                                    @foreach($venueamenities as $typename)
                                    <tr>
                                        <th scope="row">{{  $i++ }}</th>
                                        <td>{{ $typename->amenities_name }}</td>
                ...
                @endif
                <a href="{{ url('/admin/venueamenities/'.$typename->id.'/edit') }}" class="btn-warning btn" title="Edit"><i class="fa fa-pencil action_icon"></i>
                </a>
                 <button type="button" class="btn-danger btn deleteid" data-bs-toggle="modal"  data-bs-target="#delModal" data-id="{{ $typename->id }}" title="Delete"  >
                    <i class="fa fa-trash action_icon"></i>
                                    ...
                                    @endforeach
                                     {{ $venueamenities->links('pagination::bootstrap-4') }}
...
@endsection
<input type="hidden" name="redirecturl" id="redirecturl" value="{{ url('/admin/venueamenities/') }}">  
