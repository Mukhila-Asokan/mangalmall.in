        ...
        <div class="text-end">
            ...
            <form class="form-horizontal" role="form" method = "post" action="{{ route('roles.update') }}">
                        ...
                        <div class="mb-4 row">
                            ...
                            <div class="col-md-8">
                                    <input type="text" id="rolename" name="rolename" class="form-control" placeholder="Enter the Role name" value = "{{ $roles->rolename }}" required>
                                    @error('rolename')
                            ...
                            <div class="col-md-8">
                                <select id="departmentid" name="departmentid" class="form-control" >
                                    ...
                                    @foreach($department as $dept)                                    
                                        ...
                                        <option value="{{ $dept->id }}" {{ ($dept->id == $roles->departmentid) ? "Selected" : ''; }}> {{ $dept->departmentname }}</option> 
                                    @endforeach
                                </select>
                                @error('departmentid')
                                       <div class="text-danger">{{ $message }}</div>
